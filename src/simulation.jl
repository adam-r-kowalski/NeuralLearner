struct Simulation{Environment, Agent}
    ...
    Simulation(env::Environment{ObservationSpace, ActionSpace},
               agent::Agent{Observation, Action, ObservationSpace, ActionSpace}
               ) where {ObservationSpace, ActionSpace, Observation, Action} =
        new{typeof(env), typeof(agent)}(env, agent)
end
